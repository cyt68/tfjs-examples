...
# data_dir = 'data'
# # 路径为存放数据集解压后的.gnt文件
# train_data_dir = os.path.join('', 'HWDB1.1tst_gnt')
# test_data_dir = os.path.join('', 'HWDB1.1tst_gnt')
...
#                 break
#             image = np.fromfile(f, dtype='uint8', count=width *
...
def copyFiles(src, tar):
  ...
  for dir in dirs: #遍历文件夹
    if os.path.isdir(src + dir):
      if not os.path.exists(tar + dir):
        os.mkdir(tar + dir)
      files = os.listdir(src + dir)
      for index in range(10):
        shutil.copyfile(src + dir + '/' + files[index], tar + dir + '/' + files[index])

copyFiles('mnist-node/img/train1/', 'mnist-node/img/test1/')
